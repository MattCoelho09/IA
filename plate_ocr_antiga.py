...
class PlateOcrAntiga:
    def __init__(self):
        ...
        self.dnn=False,
        self.data='data/data_antiga.yaml',
        self.imgsz=640, 
        self.half=False,
        self.model = DetectMultiBackend('weigth_types/exp70/weights/best.pt', device=select_device('cpu'), dnn=self.dnn, data=self.data[0])
    ...
    def ReadPlateAntiga(self,img):
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                arr = np.asarray(array_teste)
                arrList = arr.tolist()
                numbers = []
                letters =[]
                for letra in arrList:
                    i=i+1
                    print(letra[0])
                    if(i>3):
                        numbers.append(letra[0])
                for letra in arrList:
                    print(letra[0])
                    if(k<3):
                        letters.append(letra[0])
                    k=k+1
                print(numbers) 
                 
                ## PART NUMBERS
                newList = list(map(lambda x:x.replace('I','1'),numbers))
                newList2 = list(map(lambda x:x.replace('O','0'),newList))
                hehe = str(newList2)
                a = ''.join(hehe.replace("'",""))
                b = ''.join(a.replace(" ",""))
                c = ''.join(b.replace("[",""))
                d = ''.join(c.replace("]",""))
                part_number = ''.join(d.replace(",",""))
                print(part_number)
                     
                ## PART letters
                newLetters = list(map(lambda x:x.replace('4','A'),letters))
                newLetters2 = list(map(lambda x:x.replace('8','B'),newLetters))
                kk = str(newLetters2)
                e = ''.join(kk.replace("'",""))
                f = ''.join(e.replace(" ",""))
                g = ''.join(f.replace("[",""))
                h = ''.join(g.replace("]",""))
                part_letters= ''.join(h.replace(",",""))
       
                print(part_letters)
                plate_text = part_letters + part_number
                print(plate_text)
